...
describe '投稿のテスト' do
  let!(:list) { coreate(:list,title:'hoge', body:'body')}
  describe：'トップ画面(top_path)のテスト' do
    ...
    context '表示の確認' do
      it 'トップ画面(top_path)に「ここはTopページです」が表示されているか' do
        expect(page).to have_content 'ここtはTopページです'
      end
      it：'top_pathが"/top"であるか' do
        expect(curent_path).to eq('/top')
      end
    end
  end
  describe '投稿画面のテスト' do
    ...
    context '表示の確認' do 
      it 'todolists_new_pathが"/todolists/new"であるか' do
        expext(current_path).to eq('/todolists/new')
      end
